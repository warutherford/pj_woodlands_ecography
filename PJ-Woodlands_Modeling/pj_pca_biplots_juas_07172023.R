...
jude_fit <- pj_samps_jude %>% dplyr::select(diurnalRange,
                                     pptWarmQuart,
                                     usgs_aspect,
                                     meanTempWetQuart,
...
# Preprocess data
pj_rec_jude <-  recipe(~diurnalRange+ 
                       pptWarmQuart+ 
                       usgs_aspect+ 
                       meanTempWetQuart+ 
                       annualMeanTemperature+ 
...
juas_biplot <- pj_tidied_pca_rot_wide %>%
  ggplot(aes(PC1, PC2)) +
  ...
  geom_text(
    ...
    hjust = 0, nudge_x = 0, nudge_y = -.7, size = 6, angle = 35,
    color = "darkorange4") + 
  ggtitle("JUAS")+
...
ggsave(filename = "Graphs/juas_biplot_120123.tiff",
       plot = juas_biplot,
       dpi = 800, 
       width = 8,
       height = 6,
