...
pj_samps_pied <- pj_data_all_clean  %>% mutate(across(where(is.numeric),~replace_na(.,0)),
...
pied_fit <- pj_samps_pied %>% dplyr::select(pptSeasonality,
                                     meanTempDryQuart,
                                     om_depth_15_30,
                                     meanTempWetQuart,
                                     diurnalRange,
...
pied_all<-pied_fit %>%
  augment(pj_samps_pied)

pied_all %>% # add original dataset back in
...
pied_fit %>%
  ...
  geom_text(
    ...
    color = "#904C2F", size =5) +
  #geom_point(aes(Longitude_NAD83, Latitude_NAD83), data = pied_all)
...
#data_split <- initial_split(pj_samps_pied, prop = 0.70)
#pj_train <- training(data_split)
#pj_test <- testing(data_split)

# Preprocess data
pj_rec_pied <-  recipe(~pptSeasonality+
                       meanTempDryQuart+
                       om_depth_15_30+
                       meanTempWetQuart+
                       diurnalRange+
...
ggsave(filename = "Graphs/pied_biplot_120123.tiff",
       plot = pied_biplot,
       dpi = 800, 
       width = 8,
       height = 6,
