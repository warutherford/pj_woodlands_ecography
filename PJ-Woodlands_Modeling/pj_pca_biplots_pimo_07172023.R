...
pj_samps_pimo <- pj_data_all_clean  %>% mutate(across(where(is.numeric),~replace_na(.,0)),
...
pimo_fit <- pj_samps_pimo %>% dplyr::select(tempseasonality,
                                     pptColdQuart,
                                     pptWarmQuart,
                                     meanTempWetQuart,
                                     maxTempWarmMonth,
...
pimo_all<-pimo_fit %>%
  augment(pj_samps_pimo)

pimo_all %>% # add original dataset back in
...
pimo_fit %>%
  ...
  geom_text(
    ...
    color = "#904C2F", size =5) +
  #geom_point(aes(Longitude_NAD83, Latitude_NAD83), data = pimo_all)
...
#data_split <- initial_split(pj_samps_pimo, prop = 0.70)
#pj_train <- training(data_split)
#pj_test <- testing(data_split)

# Preprocess data
pj_rec_pimo <-  recipe(~tempseasonality+ 
                       pptColdQuart+ 
                       pptWarmQuart+ 
                       meanTempWetQuart+ 
                       maxTempWarmMonth+ 
...
ggsave(filename = "Graphs/pimo_biplot_120123.tiff",
       plot = pimo_biplot,
       dpi = 800, 
       width = 8,
       height = 6,
