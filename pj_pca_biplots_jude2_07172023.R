...
pj_samps_jude <- pj_data_all_clean  %>% mutate(across(where(is.numeric),~replace_na(.,0)),
  ...
  droplevels()%>%
  filter(species == "JUDE2") %>% 
  droplevels()

jude_fit <- pj_samps_jude %>% dplyr::select(silt_depth_30_60,
                                     ksat_depth_0_5,
                                     ph_depth_100_200,
                                     meanTempWetQuart,
                                     thetas_depth_30_60,
...
jude_all<-jude_fit %>%
  augment(pj_samps_jude)

jude_all %>% # add original dataset back in
...
jude_fit %>%
  ...
  geom_text(
    ...
    color = "#904C2F", size =5) +
  #geom_point(aes(Longitude_NAD83, Latitude_NAD83), data = jude_all)
...
#data_split <- initial_split(pj_samps_jude, prop = 0.70)
#pj_train <- training(data_split)
#pj_test <- testing(data_split)

# Preprocess data
pj_rec_jude <-  recipe(~silt_depth_30_60+ 
                       ksat_depth_0_5+ 
                       ph_depth_100_200+ 
                       meanTempWetQuart+ 
                       thetas_depth_30_60+ 
...
ggsave(filename = "Graphs/jude_biplot_120123.tiff",
       plot = jude_biplot,
       dpi = 800, 
       width = 8,
       height = 6,
