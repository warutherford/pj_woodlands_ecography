...
juoc_fit <- pj_samps_juoc %>% dplyr::select(usgs_aspect,
                                     tempAnnualRange,
                                     ksat_depth_100_200,
                                     pptSeasonality,
                                     dep_bedrock,
...
juoc_all<-juoc_fit %>%
  augment(pj_samps_juoc)

juoc_all %>% # add original dataset back in
...
juoc_fit %>%
  ...
  geom_text(
    ...
    color = "#904C2F", size =5) +
  #geom_point(aes(Longitude_NAD83, Latitude_NAD83), data = juoc_all)
...
#data_split <- initial_split(pj_samps_juoc, prop = 0.70)
#pj_train <- training(data_split)
#pj_test <- testing(data_split)

# Preprocess data
pj_rec_juoc <-  recipe(~usgs_aspect+
                       tempAnnualRange+
                       ksat_depth_100_200+
                       pptSeasonality+
                       dep_bedrock+
...
juoc_biplot <- pj_tidied_pca_rot_wide %>%
  ggplot(aes(PC1, PC2)) +
  ...
  geom_text(aes(10*PC1, 10*PC2,label = terms, fontface = "bold"),
    hjust = 0, nudge_x =0.3, nudge_y = 0, size = 5.5, angle = 4,
    color = "darkorange") +
  # geom_text(aes(10*PC1, 10*PC2,label = terms, fontface = "bold"),
...
ggsave(filename = "Graphs/juoc_biplot_120123.tiff",
       plot = juoc_biplot,
       dpi = 800, 
       width = 8,
       height = 6,
