...
pj_samps_juos <- pj_data_all_clean  %>% mutate(across(where(is.numeric),~replace_na(.,0)),
...
juos_fit <- pj_samps_juos %>% dplyr::select(meanTempDryQuart,
                                     pptWarmQuart,
                                     meanTempWetQuart,
                                     dep_bedrock,
                                     tempAnnualRange,
                                     ...
                                     silt_depth_30_60,
                                     diurnalRange,
                                     pptWetMonth) %>% prcomp(scale = TRUE)

juos_all<-juos_fit %>%
  augment(pj_samps_juos)

juos_all %>% # add original dataset back in
...
juos_fit %>%
  ...
  geom_text(
    ...
    color = "#904C2F", size =5) +
  #geom_point(aes(Longitude_NAD83, Latitude_NAD83), data = juos_all)
  xlim(-.6, .6) + ylim(-1, 0.5)
...
data_split <- initial_split(pj_samps_juos, prop = 0.70)
pj_train <- training(data_split)
pj_test <- testing(data_split)

# Preprocess data
pj_rec_juos <-  recipe(~meanTempDryQuart+
                       pptWarmQuart+
                       meanTempWetQuart+
                       dep_bedrock+
                       tempAnnualRange+
...
juos_biplot <- pj_tidied_pca_rot_wide %>%
  ggplot(aes(PC1, PC2)) +
  #geom_point(aes(PC01, PC02, color = species), alpha = 0.7, size = 2, data = pj_pca_baked) +
  #scale_color_manual(values = "chocolate1")+
  geom_segment(aes(10*PC1, 10*PC2), xend = 0, yend = 0, arrow = arrow_style, size = 2, color = "black") +
  geom_text(aes(10*PC1, 10*PC2,label = terms, fontface = "bold"),
    hjust = 0, nudge_x = 0, nudge_y = 1, size = 6, angle = 20,
    color = "lightsalmon3") +
  # geom_text(aes(10*PC1, 10*PC2,label = terms, fontface = "bold"),
  #           hjust = 0, nudge_x = -0.8, nudge_y = 1,
  #           color = "black", size =5) + 
  ggtitle("JUOS")+
...
ggsave(filename = "Graphs/juos_biplot_120123.tiff",
       plot = juos_biplot,
       dpi = 800, 
       width = 8,
       height = 6,
