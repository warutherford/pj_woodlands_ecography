...
pj_samps_juvi <- pj_data_all_clean  %>% mutate(across(where(is.numeric),~replace_na(.,0)),
...
juvi_fit <- pj_samps_juvi %>% dplyr::select(pptWarmQuart,
...
juvi_all<-juvi_fit %>%
  augment(pj_samps_juvi)

juvi_all %>% # add original dataset back in
...
juvi_fit %>%
  ...
  geom_text(
    ...
    color = "#904C2F", size =5) +
  #geom_point(aes(Longitude_NAD83, Latitude_NAD83), data = juvi_all)
...
#data_split <- initial_split(pj_samps_juvi, prop = 0.70)
#pj_train <- training(data_split)
#pj_test <- testing(data_split)

# Preprocess data
pj_rec_juvi <-  recipe(~pptWarmQuart+
                       thetas_depth_100_200+
                       meanTempWetQuart+
                       dep_bedrock+
                       ph_depth_0_5+
...
pj_pca_prep <- prep(pj_rec_juvi)

pj_pca_baked <- bake(pj_pca_prep, new_data = NULL)

pj_tidied_pca_var <- tidy(pj_pca_prep, 6, type = "variance")

pj_tidied_pca_coeff <- tidy(pj_pca_prep, 6, type = "coef")
...
juvi_biplot <- pj_tidied_pca_rot_wide %>%
  ggplot(aes(PC1, PC2)) +
 # geom_point(aes(PC01, PC02, color = species), alpha = 0.7, size = 2, data = pj_pca_baked) +
 # scale_color_manual(values = "indianred2")+
  geom_segment(aes(10*PC1, 10*PC2), xend = 0, yend = 0, size =2, arrow = arrow_style) +
  geom_text(
    aes(10*PC1, 10*PC2,label = terms, fontface = "bold"),
    hjust = 0, nudge_x = 0, nudge_y = 0.8, size = 8, angle = 20,
    color = "indianred2") + 
  ggtitle("JUVI")+
  #stat_ellipse(mapping = aes(PC01, PC02),data = pj_pca_baked) +
  xlim(-5, 8) + ylim(-8, 8) +
  #coord_fixed()+
...
ggsave(filename = "Graphs/juvi_biplot_21424.tiff",
       plot = juvi_biplot,
       dpi = 800, 
       width = 10,
       height = 8,
...
juvi_biplot_13 <- pj_tidied_pca_rot_wide %>%
  ggplot(aes(PC1, PC3)) +
  # geom_point(aes(PC01, PC02, color = species), alpha = 0.7, size = 2, data = pj_pca_baked) +
  # scale_color_manual(values = "indianred2")+
  geom_segment(aes(10*PC1, 10*PC3), xend = 0, yend = 0, size =2, arrow = arrow_style) +
  geom_text(
    aes(10*PC1, 10*PC3,label = terms, fontface = "bold"),
    hjust = 0, nudge_x = 0, nudge_y = 0.8, size = 6, angle = 20,
    color = "indianred2") + 
  ggtitle("JUVI")+
  #stat_ellipse(mapping = aes(PC01, PC02),data = pj_pca_baked) +
  xlim(-5, 8) + ylim(-8, 8) +
...
ggsave(filename = "Graphs/juvi_biplot_13_120723.tiff",
       plot = juvi_biplot,
       dpi = 800, 
       width = 8,
       height = 6,
