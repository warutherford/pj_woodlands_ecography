...
pj_samps_jusc <- pj_data_all_clean  %>% mutate(across(where(is.numeric),~replace_na(.,0)),
  ...
  droplevels()%>%
  filter(species == "JUSC2") %>% 
  droplevels() %>% 
  filter(!AssocKey %in% c("JUSC2_4", "JUSC2_5")) %>%  #not enough data
  droplevels() 


jusc_fit <- pj_samps_jusc %>% dplyr::select(diurnalRange,
                                     dep_bedrock,
                                     meanTempWetQuart,
                                     tempAnnualRange,
                                     pptDryMonth,
...
jusc_all<-jusc_fit %>%
  augment(pj_samps_jusc)

jusc_all %>% # add original dataset back in
...
jusc_fit %>%
  ...
  geom_text(
    ...
    color = "#904C2F", size =5) +
  #geom_point(aes(Longitude_NAD83, Latitude_NAD83), data = jusc_all)
...
#data_split <- initial_split(pj_samps_jusc, prop = 0.70)
#pj_train <- training(data_split)
#pj_test <- testing(data_split)

# Preprocess data
pj_rec_jusc <-  recipe(~diurnalRange+
                       dep_bedrock+
                       meanTempWetQuart+
                       tempAnnualRange+
                       pptDryMonth+
...
jusc_biplot <- pj_tidied_pca_rot_wide %>%
  ggplot(aes(PC1, PC2)) +
  ...
  geom_text(
    ...
    hjust = 0, nudge_x = 0, nudge_y = -0.5, size = 6, angle = 15,
    color = "lightblue") + 
  ggtitle("JUSC2")+
...
ggsave(filename = "Graphs/jusc_biplot_120123.tiff",
       plot = jusc_biplot,
       dpi = 800, 
       width = 8,
       height = 6,
